<template>
  <n-layout v-resize has-sider @resize="handleResize">
    <n-layout-sider
    ...
    >
      ...
      <div v-if="!collapsed" class="content-bottom">
        {{ byteConvert(aboutInfo?.quota.usage) }} / {{ byteConvert(aboutInfo?.quota.limit) }}
        <n-text type="primary" @click="showCode = true">会员码</n-text>
        <n-progress
        ...
        <p style="margin-bottom: 0">
          <n-tooltip placement="right">
            <template #trigger>
              <a style="color: #306eff" target="_blank" href="https://www.tjsky.net/?p=220#i-21"
                >文件列表刷不出来？</a
              >
            </template>
      ...
      <div v-if="!collapsed" class="sider-bottom" :class="{ vip: vipInfo?.status === 'ok' }">
        <div class="bottom-user-info">
          <img
            v-if="vipInfo?.status === 'ok'"
  ...
  <n-modal v-model:show="showCode">
    <n-card style="width: 600px" title="会员码">
      ...
      <n-input v-model:value="code" placeholder="会员码"></n-input>
      <template #action>
...
import { Camera, CircleX, Copy, File, Logout, Settings, Trash, Video } from '@vicons/tabler';
import {
  MenuOption,
  NButton,
  NCard,
...
const vipInfo = ref();
const getVip = () => {
  http.get('https://api-drive.mypikpak.com/drive/v1/privilege/vip').then((res: any) => {
...
const postCode = () => {
  http
    .post('https://api-drive.mypikpak.com/vip/v1/order/activation-code', {
...
const logoutPost = () => {
  dialog.warning({
    ...
    onPositiveClick: () => {
      http
        .post('https://user.mypikpak.com/v1/auth/revoke', {})
        .then((res) => {

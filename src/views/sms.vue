      ...
      <div class="login-box">
        ...
        >
          <n-form-item path="email">
            <n-input
              v-model:value="loginData.phone_number"
              placeholder="请输入区号加手机号如+8618513579246"
            ></n-input>
          </n-form-item>
          <n-form-item path="verification_code">
            <n-input-group>
              <n-input
                v-model:value="loginData.verification_code"
                placeholder="请输入验证码"
              ></n-input>
              <n-button :disabled="time < 60" :loading="codeLoading" @click="sendCode">{{
                time >= 60 ? '发送验证码' : '重新发送 ' + time + 's'
              }}</n-button>
            </n-input-group>
          </n-form-item>
          <n-form-item v-if="!isUser" label="">
            <n-checkbox v-model:checked="invite">接受邀请获得10天vip</n-checkbox>
          </n-form-item>
          <n-form-item>
            <n-button type="primary" class="block" :loading="loading" @click="register"
              >登录</n-button
            >
          </n-form-item>
        </n-form>
...
const initCaptcha = (action?: string) => {
  return http
    .post('https://user.mypikpak.com/v1/shield/captcha/init?client_id=YNxT9w7GMdWvEOKa', {
...
const sendCode = () => {
  if (!loginData.value.phone_number) {
    ...
    initCaptcha('POST:/v1/auth/verification')
      .then(() => {
        http
          .post('https://user.mypikpak.com/v1/auth/verification?client_id=YNxT9w7GMdWvEOKa', {
            captcha_token: loginData.value.captcha_token,
            client_id: 'YNxT9w7GMdWvEOKa',
            phone_number: loginData.value.phone_number,
            locale: 'zh-cn',
...
const register = (e: Event) => {
  ...
  formRef.value.validate((errors: any) => {
    if (!errors) {
      loading.value = true;
      http
        .post('https://user.mypikpak.com/v1/auth/verification/verify?client_id=YNxT9w7GMdWvEOKa', {
          client_id: 'YNxT9w7GMdWvEOKa',
          verification_id: loginData.value.verification_id,
          verification_code: loginData.value.verification_code,
        })
        .then((res: any) => {
          initCaptcha(isUser.value ? 'POST:/v1/auth/signin' : 'POST:/v1/auth/signup').then(() => {

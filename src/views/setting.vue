  ...
  <div class="list-page">
    <n-collapse :default-expanded-names="['-1', '0', '2', '3', '4']">
      <n-collapse-item name="-1">
        <template #header>
          绑定telegram
          <a href="https://www.tjsky.net/?p=220#Telegram" target="_blank" @click.stop="">
      ...
      <n-collapse-item name="0" title="aria2设置">
        <template #header
          >aria2设置
          <a href="https://www.tjsky.net/?p=220#arai2" target="_blank" @click.stop="">
            <n-icon style="vertical-align: middle" size="20" color="#d03050"
              ><zoom-question></zoom-question
            ></n-icon> </a
        ></template>
        <n-form label-width="100px" label-align="left" label-placement="left">
          ...
          <n-alert
            v-if="
              aria2Data.host &&
              aria2Data.host.indexOf('https://') === -1 &&
              aria2Data.host.indexOf('http://localhost') == -1 &&
              aria2Data.host.indexOf('http://127.0.0.1') === -1
            "
            title="由于浏览器限制，请按下图设置开始混合模式"
            type="info"
          >
      ...
      <n-collapse-item name="1" title="自动登录设置">
        <template #header
          >自动登录设置
          <a href="https://www.tjsky.net/?p=220#i-6" target="_blank" @click.stop="">
            <n-icon style="vertical-align: middle" size="20" color="#d03050"
              ><zoom-question></zoom-question
            ></n-icon> </a
        ></template>
      ...
      <n-collapse-item title="PC端IDM支持" name="4">
        <n-space>
          <a href="https://github.com/MotooriKashin/ef2/releases" target="_blank" class="n-button"
            >地址一：下载支持插件</a
          >
          <a
            href="https://url71.ctfile.com/f/21226171-531688310-489b35"
            target="_blank"
            class="n-button"
            >地址二：下载支持插件（密码pikpak）</a
          >
          <a href="https://www.tjsky.net/?p=220#PCIDM" target="_blank" class="n-button">使用教程</a
          ><br />先用第一个下载地址，如果你实在无法访问github再使用第二个下载地址，调用IDM下载需要配合“自定义菜单”功能实现，请查看上方的使用教程
        </n-space>
        <br />
      </n-collapse-item>
...
const goTelegram = () => {
  ...
  let matchArray =
    telegramUrl.value && decodeURIComponent(telegramUrl.value).match(/(^|&)token=([^&]*)(&|$)/);
  ...
  http
    .post('https://api-drive.mypikpak.com/user/v1/bind', {

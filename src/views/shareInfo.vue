<template>
  <n-spin :show="loading" description="获取中">
    <div class="share-info-page">
      <div v-if="shareInfo?.share_status === 'OK'" class="header">
        <div class="title n-ellipsis">
          ...
          <n-text depth="3" style="font-size: 12px">
            失效时间：
            {{
              shareInfo?.expiration_left === '-1' ? '永久有效' : shareInfo?.expiration_left + '天'
            }}
            失效次数：{{
              shareInfo?.restore_count_left === '-1'
                ? '无限次'
                : shareInfo?.restore_count_left + '次'
            }}
          </n-text>
        </div>
        <div class="action">
          <n-button type="primary" @click="restore">{{
            checkedRowKeys.length ? '转存已选' : '转存全部'
          }}</n-button>
        </div>
      </div>
      <template
        v-if="
          shareInfo?.share_status === 'PASS_CODE_EMPTY' ||
          shareInfo?.share_status === 'PASS_CODE_ERROR'
        "
      >
            ...
            <n-form-item label="请输入提取码">
              <n-input-group>
                <n-input
                  ...
                  @keyup.enter="getInfo"
                ></n-input>
                <n-button type="primary" :loading="loading && !!password" @click="getInfo"
                  >提取文件</n-button
                >
              </n-input-group>
            </n-form-item>
  ...
  (
    http.get(
      'https://api-drive.mypikpak.com/drive/v1/share' +
        (route.query.id && shareInfo.value?.pass_code_token ? '/detail' : ''),
      {
        params: {
          share_id: id.value,
...
const postShare = () => [
  http
    .post('https://api-drive.mypikpak.com/drive/v1/share', {
      ...
      file_ids: [
        'VMqQ3U0hADAqJ0hiTMAfWVS1o1',
        'VMqQ3U0WADAqJ0hiTMAfWVRyo1',
        'VMqQ3U0KADAqJ0hiTMAfWVRuo1',
      ],
    })
    .then((res) => {

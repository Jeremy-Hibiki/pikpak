<template>
  <div class="list-page list-page-files">
    <div class="header">
      ...
      <div class="action">
        <n-space>
          <n-button v-if="moveFiles?.length" type="default" @click="movePost">
    ...
    <n-modal v-model:show="showName">
      <n-card style="width: 600px" title="修改名称">
        ...
        <template v-if="newName">
          <n-input v-model:value="newName.value" :placeholder="newName.value"></n-input>
        </template>
        <template #action>
          <n-button
            :block="true"
      ...
      <n-card style="width: 600px" title="自定义菜单">
        ...
        <n-form label-width="160px" label-align="left" label-placement="left">
          ...
          <n-form-item label="自定义菜单类型：">
            <n-select v-model:value="newMenu.type" :options="menuTypeList"></n-select>
          </n-form-item>
          <n-form-item label="自定义菜单可插入：">
            <n-space>
              <template v-for="(item, k) in menuTextList" :key="k">
                <n-button @click="newMenu.content = newMenu.content + '{{' + k + '}}'">{{
                  item
                }}</n-button>
              </template>
            </n-space>
          ...
          <n-form-item label="自定义菜单内容：">
            <n-input v-model:value="newMenu.content" type="textarea"></n-input>
          </n-form-item>
          <n-form-item>

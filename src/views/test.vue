    ...
    >
      <n-upload-dragger style="width: 100vw; height: 100vh">
        ...
        <template v-else>
          ...
          <n-text type="warning" style="margin: 8px 0 0 0"> 记得解决浏览器跨域问题哦 </n-text>
        </template>
      </n-upload-dragger>
    </n-upload>
  </div>
...
const postFile = async () => {
  ...
  if (!fileInfo) {
    ...
    if (fileListRef.value) {
      for (let i in fileListRef.value) {
        if (
          fileListRef.value[i].status === 'error' &&
          (fileUpload.value[fileListRef.value[i].id].retry || 0) < 3
        ) {
          fileUpload.value[fileListRef.value[i].id].retry =
            (fileUpload.value[fileListRef.value[i].id].retry || 0) + 1;
